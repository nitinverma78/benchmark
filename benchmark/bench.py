...
# %% auto 0
__all__ = ['get_algs', 'get_moduls', 'get_scope', 'gen', 'run_alg', 'timeit', 'run', 'plot', 'benchmark', 'main']
...
def run_alg(distn, modul, typ, task, **kwargs):
...
@timeit
def run(tasks, distn, modul, typ, sub_szs, sub_algs, sub_fams, **kwargs):
    sub_tasks = [task for task in tasks if task.dat.sz in sub_szs and task.alg in sub_algs and task.dat.fam in sub_fams]
    parallel(run_alg, sub_tasks, distn, modul, typ, **kwargs, threadpool=False, progress=False, n_workers=0) # variations run in parallel depending on #cpu-cores
...
def benchmark(alg_fldr, data_sizes=[10], dist_families=["range"] , num_variations=1, **kwargs):
    typ, algs, modul, fams, sds, szs = *get_scope(alg_fldr, dist_families, num_variations), data_sizes
    ...
    distn = {};
    runs = {(sz, alg, fam): run(tasks, distn, modul, typ, [sz], [alg], [fam], **kwargs) for sz in szs for alg in algs for fam in fams}
